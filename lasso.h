...
#define STEP_TIME 0.02	//V: changed
#define LEVEL_TIME 50	//V: added
...
class Lasso : public MovingObject {
 ...
 Lasso(double speed, double angle_deg, double argax, double argay, bool argpaused, bool rtheta) : MovingObject(speed, angle_deg, argax, argay, argpaused, rtheta) {
  ...
  void yank();
  void loopit();
  void setVelocity(double speed, double angle_deg);
  void setMagnet(bool mag) { magnet=mag; }
  void getCoin();

  void nextStep(double t);
  void check_for_coin(Coin *coin);
  void addCoins(int n) { num_coins+=n; }
  int getNumCoins() { return num_coins; }
  bool isAttracting() { return magnet && lasso_looped && !isPaused(); }
  bool isMagnet() { return magnet; }
